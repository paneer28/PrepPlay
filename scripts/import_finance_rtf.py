...
RTF_PATH = Path("/Users/paneer/Downloads/All Finance.rtf")
OUTPUT_PATH = ROOT / "data" / "performance-indicators.json"


SECTION_MAP = [
    ("Business Administration Core", "tier1"),
    ("Finance Career Cluster", "tier2"),
    ("Accounting Pathway", "accounting_pathway"),
    ("Banking Services Pathway", "banking_pathway"),
    ("Corporate Finance Pathway", "corporate_finance_pathway"),
    ("Insurance Pathway", "insurance_pathway"),
    ("Securities and Investments Pathway", "securities_pathway"),
]

SECTION_EVENT_IDS = {
    "tier1": ["principles-finance", "act-series", "bfs-series", "ftdm-team"],
    "tier2": ["act-series", "bfs-series", "ftdm-team"],
    "accounting_pathway": ["act-series"],
    "corporate_finance_pathway": ["bfs-series"],
}
...
def parse_indicators(text: str):
    ...
    for raw_line in lines:
        ...
        if "Copyright" in line or line.startswith("Finance Cluster for"):
            flush_buffer(True)
            continue
...
def merge_finance_with_existing(parsed_items: list[dict[str, str]]):
    existing = json.loads(OUTPUT_PATH.read_text())
    non_finance_items = [item for item in existing if item["clusterId"] != "finance"]
    ...
    for code, item in sorted(merged_by_code.items(), key=lambda entry: entry[0]):
        ...
        generated_finance_items.append(
            {
                "id": f"pi-finance-{code.lower().replace(':', '-')}",
                "code": code,
                "text": item["text"],
                "clusterId": "finance",
    ...
    combined = non_finance_items + generated_finance_items
...
def main():
    ...
    merge_finance_with_existing(parsed_items)
